...
# LÓGICA DE SEPARACIÓN POR USUARIO
if "id_usuario" not in st.session_state:
    st.session_state.id_usuario = str(uuid.uuid4())[:8]

# Cada usuario tiene su propio archivo de datos
DB_FILE = f"datos_{st.session_state.id_usuario}.txt"
TOLERANCIA = 0.080

# --- TUS ESTILOS ORIGINALES (PROHIBIDO TOCAR DISEÑO) ---
st.markdown("""
    <style>
    .caja-anillo {
...
with st.sidebar:
    ...
    st.divider()
    clave = st.text_input("Acceso Admin", type="password")

st.title("🏆 PRUEBAS")

# PESTAÑAS DINÁMICAS (ADMIN SÓLO CON CLAVE)
tabs_titulos = ["📝 REGISTRO", "🏆 COTEJO"]
if clave == "homero2026":
    tabs_titulos.append("🕵️ ADMIN")

t = st.tabs(tabs_titulos)
...
with t[0]:
    ...
    if st.session_state.partidos:
        ...
        st.data_editor(df, use_container_width=True, hide_index=True, key="editor_v1")
        
        # EL BOTÓN DE BORRAR PARA TODOS (INVITADOS Y TÚ)
        ...
        for r in range(1, st.session_state.n_gallos + 1):
            ...
            while len(lista) >= 2:
                ...
                if v_idx is not None:
                    ...
                    html += f"<tr><td>{pelea_n}</td><td class='cuadro'>□</td><td style='border-left:3px solid red'><span class='nombre-partido'>{rojo['PARTIDO']}</span><span class='peso-texto'>{rojo[col_g]:.3f}</span></td><td>{an_r:03d}</td><td class='cuadro'>□</td><td {c}>{d:.3f}</td><td>{an_v:03d}</td><td style='border-right:3px solid green'><span class='nombre-partido'>{verde['PARTIDO']}</span><span class='peso-texto'>{verde[col_g]:.3f}</span></td><td class='cuadro'>□</td></tr>"
                    pelea_n += 1
                else: break
            st.markdown(html + "</tbody></table><br>", unsafe_allow_html=True)

# --- PESTAÑA 3: ADMIN (SOLO TÚ) ---
if clave == "homero2026":
    with t[2]:
        st.header("🕵️ Monitor Maestro")
        archivos = [f for f in os.listdir(".") if f.startswith("datos_")]
        st.write(f"Galleras usando el sistema: **{len(archivos)}**")
