...
if st.session_state.id_usuario == "":
    st.markdown("""
        ...
        .main-subtitle { font-size: 0.75rem; color: #888; text-align: center; letter-spacing: 3px; text-transform: uppercase; }
        </style>
    ...
    with col_center:
        ...
        st.markdown('<div class="desc-box"><div style="color:#E67E22; font-weight:bold; font-size:0.85rem;">¿QUÉ ES ESTE SISTEMA?</div><div style="font-size:0.75rem;">Plataforma de sorteo digital para combates gallísticos justos.</div></div>', unsafe_allow_html=True)
...
tabs_list = ["📝 REGISTRO Y EDICIÓN", "🏆 COTEJO", "📑 PROTOCOLO DE OPERACIÓN"]
if acceso_admin == "28days":
    tabs_list.append("🛠️ PANEL CONTROL ADMIN")
...
# TABS 3: PROTOCOLO / TUTORIAL
with tabs[2]:
    st.markdown("## 📖 Guía de Uso del Sistema DerbySystem")
    st.info("Siga este tutorial paso a paso para operar el evento correctamente.")
    ...
    with col_t1:
        ...
        <div class="protocol-step">
            ...
            <span class="protocol-title">Registro de Modalidad</span>
            <p>Antes de ingresar pesos, seleccione en la pestaña Registro cuántos gallos jugará cada partido (2 a 6). Una vez guardado el primer partido, esta opción se bloquea para mantener la integridad del torneo.</p>
        </div>
        <div class="protocol-step">
            <span class="protocol-number">2</span>
            <span class="protocol-title">Captura de Partidos</span>
            <p>Ingrese el nombre del partido y los pesos exactos. El sistema asignará automáticamente los números de anillos correlativos para cada ejemplar.</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_t2:
        st.markdown("""
        <div class="protocol-step">
            <span class="protocol-number">3</span>
            <span class="protocol-title">Generación de Cotejo</span>
            <p>En la pestaña 'Cotejo' podrá visualizar los emparejamientos por peso. El sistema garantiza que ningún partido pelee contra sí mismo.</p>
        </div>
        <div class="protocol-step">
            <span class="protocol-number">4</span>
            <span class="protocol-title">Impresión de Reportes</span>
            <p>Descargue el PDF oficial. Este documento contiene los espacios para firmas y el control manual de los jueces en el pesaje final.</p>
        </div>
        """, unsafe_allow_html=True)

# TABS 4: PANEL ADMIN (NUEVA UBICACIÓN)
if acceso_admin == "28days":
    with tabs[3]:
        ...
        if usuarios:
            user_data = [{"Usuario": u, "Contraseña (Hash)": p} for u, p in usuarios.items()]
            st.table(user_data)
        
        st.divider()
        
        # Mostrar datos de archivos registrados
        st.subheader("📁 Archivos de Datos (.txt)")
        archivos = [f for f in os.listdir(".") if f.startswith("datos_") and f.endswith(".txt")]
        if archivos:
            for arch in archivos:
                with st.expander(f"Datos Registrados en: {arch}", expanded=False):
                    try:
            ...
            st.write("No hay archivos de datos registrados.")
