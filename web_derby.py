...
# --- PANTALLA DE ENTRADA (NOMBRE + PIN) ---
if st.session_state.id_usuario == "":
    html_bienvenida = (
        ...
        "<div style='color:#E67E22; font-weight:bold; font-size:1.2rem; margin-bottom:10px; text-align:center;'>Seguridad de Mesa</div>"
        "<div style='color:#f2f2f2; font-size:0.95rem; line-height:1.5; text-align:center;'>"
        "Para proteger sus registros, ingrese el nombre del evento y un PIN de seguridad de 4 dígitos."
        "</div>"
        "<hr style='border:0.5px solid #333; margin:15px 0;'>"
        "<div style='font-size:0.85rem; color:#E67E22; font-style:italic; text-align:center;'>Si el evento ya existe, ingrese el mismo PIN para retomar los datos.</div>"
        "</div></div>"
    )
    ...
    with col_b:
        nombre_ev = st.text_input("NOMBRE DEL EVENTO:", placeholder="Ej. GRAN DERBY 2026").upper().strip()
        pin_ev = st.text_input("PIN DE SEGURIDAD (4 DÍGITOS):", type="password", placeholder="****")
        
        if st.button("ACCEDER AL PANEL", use_container_width=True):
            if nombre_ev and len(pin_ev) >= 4:
                # Creamos una identidad única basada en nombre y pin
                st.session_state.id_usuario = f"{nombre_ev}_{pin_ev}"
                st.rerun()
            else:
                st.warning("⚠️ Ingrese el nombre del evento y un PIN de al menos 4 números.")
    st.stop()
...
st.markdown("""
    ...
    .nombre-partido { 
        ...
        white-space: normal; word-wrap: break-word; color: black !important;
    }
    ...
    .manual-header {
        color: #1a1a1a; font-weight: bold; text-transform: uppercase;
...
# --- FUNCIÓN DE PDF (DISEÑO BLANCO Y NARANJA) ---
def generar_pdf(partidos, n_gallos):
    ...
    t_firmas = Table([["__________________________", " ", "__________________________"], ["FIRMA JUEZ DE PLAZA", " ", "FIRMA MESA DE CONTROL"]], colWidths=[200, 100, 200])
...
# Título limpio (Solo nombre del evento sin el PIN interno)
st.title(f"🏆 {st.session_state.id_usuario.split('_')[0]}")
...
with t_cot:
    if len(st.session_state.partidos) >= 2:
        ...
        for r in range(1, st.session_state.n_gallos + 1):
            ...
            html = """<table class='tabla-final'><thead><tr><th style='width:22px'>#</th><th style='width:25px'>G</th><th>ROJO</th><th style='width:35px'>AN.</th><th style='width:22px'>E</th><th style='width:45px'>DIF.</th><th style='width:35px'>AN.</th><th>VERDE</th><th style='width:25px'>G</th></tr></thead><tbody>"""
            pelea_n = 1
            while len(lista) >= 2:
                rojo = lista.pop(0)
                v_idx = next((i for i, x in enumerate(lista) if limpiar_nombre_socio(x["PARTIDO"]) != limpiar_nombre_socio(rojo["PARTIDO"])), None)
                if v_idx is not None:
                    ...
                    html += f"<tr><td>{pelea_n}</td><td class='cuadro'>□</td><td style='border-left:3px solid red'><span class='nombre-partido'>{rojo['PARTIDO']}</span><br><span class='peso-texto'>{rojo[col_g]:.3f}</span></td><td>{an_r:03}</td><td class='cuadro'>□</td><td {c}>{d:.3f}</td><td>{an_v:03}</td><td style='border-right:3px solid green'><span class='nombre-partido'>{verde['PARTIDO']}</span><br><span class='peso-texto'>{verde[col_g]:.3f}</span></td><td class='cuadro'>□</td></tr>"
                    pelea_n += 1
                else: break
...
with t_ayu:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown('<div class="manual-card"><div class="manual-header">01. INICIALIZACIÓN</div><p style="color:#333; font-size:0.85rem;">Defina la modalidad de combate (2-6 gallos). Capture nombres y pesos con 3 decimales.</p></div>', unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="manual-card"><div class="manual-header">02. ANILLOS</div><p style="color:#333; font-size:0.85rem;">Generación automática de folios según orden de registro para trazabilidad total.</p></div>', unsafe_allow_html=True)

# --- BARRA LATERAL (ADMIN) ---
with st.sidebar:
    st.write(f"Sesión: **{st.session_state.id_usuario.split('_')[0]}**")
