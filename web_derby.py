...
import pandas as pd
import os
import uuid  # Nuevo: Para separar usuarios
...
# --- LÓGICA MULTI-USUARIO (NUEVO) ---
# Crea un ID único por cada persona que entra [cite: 18-01-2026]
if "id_usuario" not in st.session_state:
    st.session_state.id_usuario = str(uuid.uuid4())[:8]

# El archivo ahora es individual por usuario [cite: 18-01-2026]
...
# --- PANEL DE ADMIN (SOLO ESTO SE AÑADIÓ AL FINAL) ---
st.sidebar.markdown("---")
# Clave: homero2026. IMPORTANTE: Escribe y presiona ENTER para activar [cite: 18-01-2026]
...
if clave == "homero2026":
    ...
    # Busca todos los archivos de sesión en el servidor [cite: 18-01-2026]
    archivos = [f for f in os.listdir(".") if f.startswith("datos_derby_")]
    st.write(f"Sesiones abiertas: **{len(archivos)}**")
    for arch in archivos:
        with st.expander(f"Datos de sesión: {arch}"):
