...
st.markdown("""
    ...
    /* ESTILO LLAMATIVO PARA EL BOTÓN DE REPORTE (PDF) */
...
def generar_pdf(partidos, n_gallos):
    ...
    # Encabezado Blanco con Logo DERBY(negro) System(naranja)
    data_header = [
        ...
        [Paragraph(f"<font color='grey' size=8>Emisión: {ahora} | Evento: {st.session_state.id_usuario}</font>", styles['Normal'])]
    ]
...
with t_ayu:
    ...
    with col_1:
        st.markdown("""<div class="manual-card"><div class="manual-header">01. INICIALIZACIÓN</div><p style='color:#333; font-size:0.85rem;'>Configure la modalidad y capture pesos con 3 decimales.</p></div>""", unsafe_allow_html=True)
    with col_2:
        st.markdown("""<div class="manual-card"><div class="manual-header">03. PROCESAMIENTO</div><p style='color:#333; font-size:0.85rem;'>Cotejo inteligente automático por proximidad de masa.</p></div>""", unsafe_allow_html=True)
