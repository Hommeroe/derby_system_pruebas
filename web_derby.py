...
# SEPARACIÓN DE USUARIOS (Sin afectar diseño)
...
with t_reg:
    ...
    with st.form("f_nuevo", clear_on_submit=True):
        ...
        for i in range(g_sel):
            ...
            # Anillo automático intacto [cite: 14-01-2026]
...
with t_cot:
    if len(st.session_state.partidos) >= 2:
        # AQUÍ ESTÁ DE VUELTA TU BOTÓN DE IMPRIMIR
...
# PANEL ADMIN (Oculto en barra lateral)
acceso = st.sidebar.text_input("Acceso Maestro:", type="password")
if acceso == "homero2026":
    st.sidebar.divider()
    archivos = [f for f in os.listdir(".") if f.startswith("datos_") and f.endswith(".txt")]
    st.sidebar.write(f"Usuarios: {len(archivos)}")
    for arch in archivos:
        with st.sidebar.expander(f"Ver: {arch}"):
