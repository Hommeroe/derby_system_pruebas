...
if st.session_state.id_usuario == "":
    ...
    with col_center:
        ...
        st.markdown('<div class="desc-box"><div style="color:#E67E22; font-weight:bold; font-size:0.85rem;">¿QUÉ ES ESTE SISTEMA?</div><div style="font-size:0.75rem;">Sorteo digital y transparencia gallística.</div></div>', unsafe_allow_html=True)
...
def generar_pdf(partidos, n_gallos):
    ...
    data_header = [[Paragraph("<font color='white' size=22><b>DerbySystem</b></font>", styles['Title'])],
                   [Paragraph("<font color='#E67E22' size=14><b>PRO MANAGEMENT</b></font>", styles['Normal'])],
    ...
    for r in range(1, n_gallos + 1):
        ...
        while len(lista) >= 2:
            ...
            if v_idx is not None:
                ...
                data.append([pelea_n, "", Paragraph(f"<b>{rojo['PARTIDO']}</b><br/>{rojo[col_g]:.3f}", styles['Normal']), f"{an_r:03}", "", f"{d:.3f}", f"{an_v:03}", Paragraph(f"<b>{verde['PARTIDO']}</b><br/>{verde[col_g]:.3f}", styles['Normal']), ""])
                pelea_n += 1
            else: break
        t = Table(data, colWidths=[20, 20, 140, 35, 20, 40, 35, 140, 20])
        t.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor("#1a1a1a")), ('TEXTCOLOR', (0,0), (-1,0), colors.HexColor("#E67E22")), ('GRID', (0,0), (-1,-1), 0.5, colors.grey), ('ALIGN', (0,0), (-1,-1), 'CENTER')]))
    ...
    if st.session_state.partidos:
        ...
        for p in st.session_state.partidos:
            ...
            for i in range(1, st.session_state.n_gallos + 1):
                item[f"G{i}"] = p[f"G{i}"]; item[f"A{i}"] = f"{c_an:03}"; c_an += 1
...
with t_cot:
    if len(st.session_state.partidos) >= 2:
        ...
        st.download_button("📥 DESCARGAR PDF", pdf_bytes, "cotejo.pdf", "application/pdf", use_container_width=True)
...
with t_ayu:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown('<div class="protocol-step"><span class="protocol-number">01</span><span class="protocol-title">Registro</span><div class="protocol-text">Defina modalidad y registre pesos. Anillos se generan solos.</div></div>', unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="protocol-step"><span class="protocol-number">02</span><span class="protocol-title">Cotejo</span><div class="protocol-text">Revise diferencias en rojo (>0.080) y descargue el PDF.</div></div>', unsafe_allow_html=True)

# --- PANEL ADMIN (FIJO Y COMPLETO) ---
with st.sidebar:
...
if acceso == "28days":
    st.divider()
    st.subheader("🛠️ CONTROL TOTAL")
    
    # Usuarios y Contraseñas
    st.write("**Base de Usuarios:**")
    usuarios = cargar_usuarios()
    if usuarios:
        st.table([{"User": k, "Pass Hash": v} for k, v in usuarios.items()])
    
    # Datos de Archivos .txt
    st.write("**Archivos de Datos:**")
    archivos = [f for f in os.listdir(".") if f.startswith("datos_") and f.endswith(".txt")]
    for arch in archivos:
        with st.expander(f"CONTENIDO: {arch}", expanded=False):
            try:
